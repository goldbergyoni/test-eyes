...
// apps/test-processing/scripts/action-deploy.ts
...
async function prepareSiteDir(siteDir) {
  await removeDir(siteDir);
  await ensureDir(siteDir);
}
async function createNoJekyllFile(dir) {
...
async function pushWithUpstream(localBranch, remoteBranch, force = false) {
  try {
...
async function removeAllTracked() {
  await runGitSafe("rm -rf .");
}

// apps/test-processing/src/deploy.ts
import path3 from "path";
async function prepareSite(distDir, dataFile, siteDir) {
  await prepareSiteDir(siteDir);
  await copyDir(distDir, siteDir);
  const dataDestDir = path3.join(siteDir, "data");
  await ensureDir(dataDestDir);
  await copyFile(dataFile, path3.join(dataDestDir, "main-test-data.json"));
}
async function deployToGitHubPages(config) {
...
async function deployDashboard(options) {
  ...
  const dataFile = path3.join(dataDir, "main-test-data.json");
...
async function main() {
  ...
  console.log("\u{1F680} Test Eyes - Deploying dashboard...");
